...
/* USER CODE BEGIN PV */
uint8_t TxAddress[] = {0x17, 0x08, 0x04, 0x11, 0x02};
uint8_t TxData[3] ={0};
uint8_t a = 0;
uint32_t VR[5];
uint32_t value[5];
...
//  Displ_BackLight('I');
  ...
  while (1)
  {
	  HAL_ADC_Start_DMA(&hadc1, VR, 5);
	  HAL_Delay(10);
//	  for(uint8_t i = 0; i < 5; i++){
//		  value[i] = map(VR[i], 0 , 4095, 0 ,1024);
//	  }
//	  check = value[3];
//	  if(check > 512){
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, DISPL_DC_Pin|RF_CSN_Pin|RF_CE_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : RF_CSN_Pin RF_CE_Pin */
